...
def load_be32(source_bytes, offset):
	return (
		(source_bytes[offset] << 24)
		| (source_bytes[offset + 1] << 16)
		| (source_bytes[offset + 2] << 8)
		| source_bytes[offset + 3]
	)


def store_be32(target_bytes, offset, value):
	target_bytes[offset] = (value >> 24) & 0xff
	target_bytes[offset + 1] = (value >> 16) & 0xff
	target_bytes[offset + 2] = (value >> 8) & 0xff
	target_bytes[offset + 3] = value & 0xff
...
def sha256_update(context, source_bytes, start, length):
	...
	if offset < end:
		...
		for i in range(context["buffer_length"]):
			context["buffer"][i] = source_bytes[offset + i]
...
def sha256_final(context):
	...
	if context["buffer_length"] > 56:
		for i in range(context["buffer_length"], 64):
			context["buffer"][i] = 0
		sha256_transform(context, context["buffer"], 0)
		context["buffer_length"] = 0

	for i in range(context["buffer_length"], 56):
		context["buffer"][i] = 0

	bit_count = context["bit_count"]
	context["buffer"][56] = (bit_count >> 56) & 0xff
	context["buffer"][57] = (bit_count >> 48) & 0xff
	context["buffer"][58] = (bit_count >> 40) & 0xff
	context["buffer"][59] = (bit_count >> 32) & 0xff
	context["buffer"][60] = (bit_count >> 24) & 0xff
	context["buffer"][61] = (bit_count >> 16) & 0xff
	context["buffer"][62] = (bit_count >> 8) & 0xff
	context["buffer"][63] = bit_count & 0xff
	sha256_transform(context, context["buffer"], 0)
...
def build_source_bytes():
	source_bytes = bytearray(TARGET_SIZE)
	state = [0x12345678]

	for i in range(TARGET_SIZE):
		source_bytes[i] = random_next(state) >> 24
